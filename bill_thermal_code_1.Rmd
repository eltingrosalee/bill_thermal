...
image_processing <- function(data=images) {
  images <- list.files(path="/Users/Mellisuga/Documents/R/bill_thermal/input", pattern= "*.jpg")
  nimages <- length(images)
  for (i in 1:nimages){
  #read Flir jpg into img
  img<-readflirJPG("images[i]", exiftoolpath="C:/")
  #get img dimensions
  dim <- dim(img)
  #get camera settings (cams) with flir settings
  cams <-flirsettings(img, exiftoolpath="C:/", camvals="")
  #save heading of camsinfo
  head <- head(cbind(cams$Info), 20)
  #save plancks from settings 
  plancks<-flirsettings(img, exiftoolpath="C:/", camvals="-*Planck*")
  unlist(plancks$Info)
